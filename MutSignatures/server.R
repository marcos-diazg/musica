   ...
   vcfs<-eventReactive(input$run,{
      inFile<-input$fileinput
         ...
         if (input$datatype=="VCF"){
            ...
            validate(
               need(length(grep(".vcf",inFile$datapath))>0 | length(grep(".txt",inFile$datapath))>0,"Please, select VCF")
            )
         ...
         if (input$datatype=="MAF"){
            ...
            #Warning for file format
            validate(
               need(length(grep(".maf",inFile$datapath))>0 | length(grep(".txt",inFile$datapath))>0,"Please, select VCF")
            )
         ...
         if (input$datatype=="TSV"){
            ...
            #Warning for file format
            validate(
               need(length(grep(".tsv",inFile$datapath))>0 | length(grep(".txt",inFile$datapath))>0,"Please, select VCF")
            )
         ...
         if (input$datatype=="Excel"){
            ...
            #Warning for file format
            validate(
               need(length(grep(".xlsx",inFile$datapath))>0 | length(grep(".xls",inFile$datapath))>0,"Please, select VCF")
            )
