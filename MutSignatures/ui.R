...
shinyUI(fluidPage(
   ...
   sidebarLayout(
      sidebarPanel(
         ...
         hr(),
         radioButtons("datatype", "Format", c("vcf","tsv","Excel"),selected = "vcf",inline = TRUE),
         
         #Help menu for format of input file
         actionLink("helpformat","Help with format of input file"),
         bsModal("modal","HELP: Input file format","helpformat",
                 HTML(
                     ...
                     <p>
                     <u>tsv</u>: Tab Separated Values file with four required fields. A header line is mandatory and the four headers must be provided as listed below:</p>
                     <ol>
         ...
         busyIndicator("Calculation in progress",wait=1),
